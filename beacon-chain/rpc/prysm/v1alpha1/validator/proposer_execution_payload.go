...
func (vs *Server) getLocalPayload(ctx context.Context, blk interfaces.ReadOnlyBeaconBlock, st state.BeaconState) (interfaces.ExecutionData, bool, error) {
	...
	proposerID, err := helpers.BeaconProposerIndexAtSlot(ctx, st, slot)
	if err != nil {
		logrus.WithFields(logFields).Warn("could not compute proposer index")
	}
	...
	if ok && proposerID == vIdx && payloadId != [8]byte{} {
